...
The chart below shows DC's youth population between 1990 and 2025. After falling between 1995 and 2010, the number has ticked up again, with a COVID-induced dip recently. (Tax returns show slightly different totals; see appendix).
...
The table below shows population trends by age group. The population of very young children has grown, while other age groups' populations have declined somewhat (although 2020 may have been an especially low year). The overall change from 2000 to 2020 was +278.
...
decomp <- function(row, p) {
  ...
  agedout <- e["15"] + e["20"]
  netmig  <- m["0"] + m["5"] + m["10"]
  p_end   <- sum(e[c("0", "5", "10")] + m[c("0", "5", "10")]) # end of period value is expected pop + # migrants
  tibble(births = births, agedout = -agedout, netmig = netmig,
...
The chart below uses Census data analyzed by the University of Wisconsin's [Applied Population Laboratory](https://netmigration.wisc.edu/) to decompose changes in DC's youth population into three categories:

* *The number of births*
    * This increased from `r fmt(tab$births[tab$decade=="2000s"])` in the 2000s to `r fmt(tab$births[tab$decade=="2010s"])` in the 2010s (+`r fmt(births_increase)`, or `r percent_increases[1]`% of the total increase from the 2000s to the 2010s).


* *The number of children aging out of childhood*
    * This increased from `r fmt(tab$agedout[tab$decade=="2000s"])` in the 2000s to `r fmt(tab$agedout[tab$decade=="2010s"])` in the 2010s (+`r fmt(ageout_increase)`, or `r percent_increases[2]`% of the '00-'20 total increase).


* *Less child net out-migration*
    * This increased from `r fmt(tab$netmig[tab$decade=="2000s"])` in the 2000s to `r fmt(tab$netmig[tab$decade=="2010s"])` in the 2010s (+`r fmt(migrat_increase)`, or `r percent_increases[3]`% of the '00-'20 total increase).


*More on births*

As we'll see below, this change wasn't due to an increase in the fertility rate (or the likelihood that a given person decides to have children). DC's fertility rate has fallen almost every year since the mid-2000s, which itself represented a decline from the early 1990s. The reason births increased was that DC's overall population was increasing, as net migration in each year became positive (or at least less negative). In other words, there were just more parents and soon-to-be parents living in DC during the 2010s.

It is also worth noting that whether "births increased" depends on which year you use as the base year. Births increased from the 2000s to the 2010s. But births only increased by `r fmt(tab$births[tab$decade=="2010s"] - tab$births[tab$decade=="1990s"])` from the 1990s to the 2010s. The general population increase counteracted the falling fertility rate.

*More on aging out*

Of the three factors, this was the single biggest contributor to the child population increase. This increase isn't exactly a policy achievement. It's more a statistical artifact that exists because there just weren't that many 5-to-14-year-olds living in DC in 2010, who would age out of childhood by 2020. And that is largely because the 2000s were the nadir of DC's youth-age population. So the 2010s increase is partly due to the fact that DC started from a low base number of children in the 2000s. See the appendix for more.

*More on net migration*

The 2010s saw more positive youth net migration (though it was still negative for all youth ages). The second chart below shows these trends for all age groups. See the appendix for more.
...
# Decomposing youth population changes during the 2010s: race

<br>

While the numbers of Hispanic and White children have risen in DC since 2010, the number of Black children has continued to fall. These trends also appear in school enrollment data (see appendix).
...
Part of what is likely happening here is that white parents are more likely to move to the District (or remain in DC, if they were existing residents) than they were 40 years ago. This would be part of a broader trend: the reversal of white flight in large U.S. urban areas. Meanwhile, higher housing costs have led (or financially forced) many Black residents to move to the suburbs.[^1]

[^1]: See Yesim Sayin, Kids in the Neighborhood: The District Has More Children, But They Are Not Where They Used to Be. <https://ora-cfo.dc.gov/blog/kids-neighborhood-district-has-more-children-they-are-not-where-they-used-be>
...
# Decomposing youth population changes during the 2010s: income

Since the mid-2000s, the proportion of DC children in higher-income families has increased significantly, as a share of all children.
...
a <- ggplot(results %>% filter(inc_grp=="200%+")
  ...
  geom_text(vjust=-1, aes(label=paste0(round(share*100), "%"))) +
  labs(x="", y="% of children in families above 200% AMI") +
  lims(y=c(0, 20)) +
  ggtitle("Children in families above 200% AMI almost doubled, as share of total children")
...
c <- ggplot(results, aes(x=factor(year), fill=factor(inc_grp))) +
  ...
  labs(x="", fill="Income rel. to AMI", y="% of children in families by % of AMI") +
...
The chart below shows the number of births in DC over time. Births in DC increased modestly during the 2010s, then fell again.[^2]

[^2]: See also <https://edscape.dc.gov/page/pop-and-students-number-births-and-birth-rates>
...
Demographers calculate the "general fertility rate" by dividing the births by the female population ages 15 to 44, and multiplying by 1,000.[^3] The chart below shows DC's GFR over time. This has fallen continuously.

[^3]: <https://www.ncbi.nlm.nih.gov/books/NBK215693/>
...
The chart below shows marriage and divorce counts.^[National Center for Family & Marriage Research (2023). County-level marriage and divorce data, United States, 2011-2019, (Version 1.0). Marriage, Divorce, & Birth Data Compass. Bowling Green State University. Retrieved from <http://www.bgsu.edu/ncfmr/resources/data/original-data/county-level-marriage-divorce-data-2011-2019.html>]
...
temp <-
  ...
  mutate(
    city = case_when(
      city == "Washington"~"DC",
      city == "San Francisco"~"San Fran.",
      city == "Baltimore"~"Balt.",
      city == "Philadelphia"~"Phila.",
      city == "Boston"~"Boston"
    )
  )
...
a <- ggplot(temp, aes(x=city, y=m_rate)) +
  ...
  labs(x="", y="Mar./1000 residents", 
       title="But few marriages/1000 residents") 

b <- ggplot(temp, aes(x=city, y=d_rate)) +
  geom_bar(stat = "identity") +
  scale_y_continuous(limits=c(0, 10)) +
  labs(x="", y="Div./1000 residents", 
...
The chart below shows more detail on the distribution of family income in DC over time.
...
Total enrollment in DC Public Schools (including charter schools) has risen from roughly `r fmt(tot_enroll$Total[tot_enroll$school_year == 1990])` to `r fmt(tot_enroll$Total[tot_enroll$school_year == 2023])` since 1990.[^5] All of this increase came after 2010. Black student enrollment fell, while Hispanic and White student enrollment increased significantly.[^6]

[^5]: U.S. Department of Education, National Center for Education Statistics' [Common Core of Data](https://nces.ed.gov/ccd/elsi/tableGenerator.aspx)

[^6]: Starting around 2010, the Department of Education started listing "2 or more races as an option"; this altered totals for the other groups.
...
We can even see the reversal of white flight if look at popular baby names registered with the Social Security Administration. First names more commonly associated with white people in the Census have jumped up, after falling over the latter half of the 20th century.[^7][^8]

[^7]: Kaplan, J (2023). predictrace: Predict the Race and Gender of a Given Name Using Census and Social Security Administration Data. Version 2.0.1. URL: <https://github.com/jacobkap/predictrace>, <https://jacobkap.github.io/predictrace/>.

[^8]: Note that for some names, like "Aiden" or "Genesis" the `predictrace` package doesn't return results.
